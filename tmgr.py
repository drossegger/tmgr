#!/usr/bin/python3.4
import argparse as ap, os, sqlite3 as sql, configparser,sys
...
def start(args):
    ...
    if db.fetchall()!=[]:
        ...
        sys.exit(1)
    db.execute("INSERT INTO times(project) VALUES ('%s')"%args.project)

    db.execute("SELECT start FROM times WHERE project='%s' AND stop=NULL"%args.project)
